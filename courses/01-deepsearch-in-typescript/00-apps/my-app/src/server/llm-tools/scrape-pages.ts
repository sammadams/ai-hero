...
// scrapePages tool handler for LLM integration
export const scrapePages = cacheWithRedis(
  "scrapePages",
  async (urls: string[]): Promise<{ results: { url: string; markdown: string | null; error: string | null }[] }> => {
    const crawlResults = await bulkCrawlWebsites({ urls });
    return {
      results: crawlResults.results.map(({ url, result }) => ({
        url,
        markdown: result.success ? result.data : null,
