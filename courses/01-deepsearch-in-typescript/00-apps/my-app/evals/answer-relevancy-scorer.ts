...
/**
 * Generates evaluation statements from the model output.
 * Breaks down the output into meaningful statements for relevancy analysis.
 */
export const generateEvaluationStatementsPrompt = ({
  output,
}: {
  output: string;
}) => {
...
/**
 * System prompt for the answer relevancy evaluator.
 */
...
/**
 * Generates the evaluation prompt for scoring statement relevancy.
 */
export const generateEvaluatePrompt = ({
  input,
  statements,
}: {
  input: string;
...
/**
 * Evaluates the relevancy of a response by breaking it into statements and scoring each.
 * Uses a two-step LLM approach: first generate statements, then evaluate relevancy.
 */
export const checkAnswerRelevancy = async (opts: {
  question: string;
  answer: string;
}) => {
  // Step 1: Generate statements from the answer
  const { object: statementObject } = await generateObject({
    model: factualityModel,
    prompt: generateEvaluationStatementsPrompt({ output: opts.answer }),
    schema: z.object({
      statements: z.array(z.string()).describe("Array of meaningful statements from the text"),
    }),
  });

  const statements = statementObject.statements;

  // If no statements were generated, return a low score
  if (statements.length === 0) {
    return {
      ...
      metadata: {
        statements: [],
        verdicts: [],
        rationale: "No meaningful statements found in the answer",
      },
    };
  }

  // Step 2: Evaluate the relevancy of each statement
  const { object: verdictObject } = await generateObject({
    model: factualityModel,
  ...
  const verdicts = verdictObject.verdicts;

  // Convert verdicts to scores and calculate average
  const scores: number[] = verdicts.map((verdict) => {
...
/**
 * Answer Relevancy scorer that can be used in Evalite evaluations.
